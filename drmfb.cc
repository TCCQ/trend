 ...
 *
 * !!!!!!!!!!!!!! stolen from raw.githubusercontent.com/dvdhrm/drm-howto/docs/master/modeset-double-buffered.c
 *
...
static int modeset_prepare(int fd)
{
	...
	for (i = 0; i < res->count_connectors; ++i) {
		...
		modeset_list = dev;
    break; //We only want first device
	}
...
int fd;
//copied the first part of main
int fbSetup(std::string input) {
	int ret;
	const char *card = input.c_str();
...
//	fprintf(stderr, "using card '%s'\n", card);
  ...
  SWIDTH = 100000;
  SHEIGHT = 100000;
	//we need to setup the device we found
  modeset_list->saved_crtc = drmModeGetCrtc(fd, modeset_list->crtc);
  buf = &modeset_list->bufs[modeset_list->front_buf];
  SWIDTH = min(SWIDTH, buf->width);
  SHEIGHT = min(SHEIGHT, buf->height);
  ret = drmModeSetCrtc(fd, modeset_list->crtc, buf->fb, 0, 0,
           &modeset_list->conn, 1, &modeset_list->mode);
  return 0;
}

//another thin wrapper to my code
void fbCleanup() {
	modeset_cleanup(fd);
}

//maybe some jank about diff width/height req's, but should be fine
void setPixel(unsigned int x, unsigned int y, unsigned char r,unsigned char g,unsigned char b) {
	struct modeset_buf *buf;
  unsigned int off;
  //again, only one device
  buf = &modeset_list->bufs[modeset_list->front_buf ^ 1];
  off = buf->stride * y + x * 4;
...
void flip() {
	struct modeset_buf *buf;
  buf = &modeset_list->bufs[modeset_list->front_buf ^ 1];
  drmModeSetCrtc(fd, modeset_list->crtc, buf->fb, 0, 0, &modeset_list->conn, 1, &modeset_list->mode);
